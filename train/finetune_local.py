...
def resolve_training_backend(student: dict) -> str:
    return str(student.get("training_backend", "local_lora")).lower()
...
def build_training_args(student: dict, out_dir: Path, seed: int, torch_dtype: torch.dtype | None) -> SFTConfig:
    gradient_checkpointing = bool(student.get("gradient_checkpointing", False))
    optim_name = str(student.get("optim", "adamw_torch"))
    warmup_ratio = float(student.get("warmup_ratio", 0.0))
    weight_decay = float(student.get("weight_decay", 0.0))
...
def apply_training_mode(model: AutoModelForCausalLM, student: dict) -> tuple[AutoModelForCausalLM, str, dict | None]:
    backend = resolve_training_backend(student)
    if backend == "local_full_ft":
        return model, "SFT + Full FT", None

    if backend != "local_lora":
        raise ValueError("student.training_backend must be one of: local_lora, local_full_ft")

    if LoraConfig is None or get_peft_model is None:
        raise ImportError("LoRA training requires peft to be installed")
    ...
    return get_peft_model(model, lora_config), "SFT + LoRA", {
        "r": int(student["lora_r"]),
...
def main() -> None:
    ...
    student = cfg["student"]
    training_backend = resolve_training_backend(student)
    ...
    if bool(student.get("gradient_checkpointing", False)):
        model.config.use_cache = False
        model.gradient_checkpointing_enable()

    model, method_name, lora_metadata = apply_training_mode(model, student)

    ds = load_text_dataset(args.dataset)
    dataset_rows = len(ds)

    training_args = build_training_args(student, out_dir, args.seed, torch_dtype)
    ...
    if training_backend == "local_lora" and export_merged:
    ...
    metadata = {
        ...
        "training": {
            ...
            "gradient_checkpointing": bool(student.get("gradient_checkpointing", False)),
            "optim": str(student.get("optim", "adamw_torch")),
            "warmup_ratio": float(student.get("warmup_ratio", 0.0)),
            "weight_decay": float(student.get("weight_decay", 0.0)),
        },
        "hardware": {
        ...
        "outputs": {
            ...
            "lora_adapter_dir": str(out_dir / "lora_adapter") if training_backend == "local_lora" and export_merged else None,
    ...
    elif training_backend == "local_lora":
        print(f"Saved LoRA adapter to {out_dir / 'final'}")
    else:
        print(f"Saved full fine-tuned model to {out_dir / 'final'}")
